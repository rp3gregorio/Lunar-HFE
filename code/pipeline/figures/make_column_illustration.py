...
def draw_fluxes(ax, mid):
    ...
    ax.text(X_SPLIT - 0.04, y_dead + 0.03,
            rf"daily swing $<0.1$ K below {Z_SKIN_DEAD:.0f} cm",
...
def main() -> None:
    ...
    with plt.rc_context({"savefig.pad_inches": 0.02}):
        fig.savefig(FIGS / "fig_column_illustration.pdf")
        fig.savefig(FIGS / "fig_column_illustration.png", dpi=300)
    plt.close(fig)
    print(f"  wrote figures/fig_column_illustration.pdf + .png "
