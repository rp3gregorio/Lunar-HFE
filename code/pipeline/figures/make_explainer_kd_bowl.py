...
ROOT = find_repo_root()
_DOCS = (ROOT / "deliverables" if (ROOT / "deliverables").exists()
         else ROOT.parent / "deliverables")
OUT = _DOCS / "documents" / "aogs" / "talk_figures"


def main():
    d = json.loads((ROOT / "results" / "aogs_kd_shadowed.json").read_text())
    kd = json.loads((ROOT / "results" / "kd_retrieval_results.json").read_text())
    study = json.loads((ROOT / "results" / "aogs_density_study.json").read_text())
    win_rho3 = {}
    for sk in ("A15", "A17"):
