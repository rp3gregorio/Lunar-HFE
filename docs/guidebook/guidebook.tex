...
\emph{Left:} where the seven deep sensors lie ($0.8$--$1.4$\,m, with $1\sigma$
bars), the with- and without-bedrock models sit exactly on top of each other and
fit the data equally. \emph{Right:} the realistic column to $20$\,m --- a thin
($\sim$5\,m) regolith on solid bedrock. Without bedrock (green, all regolith) the
profile keeps rising at the geothermal gradient $Q_b/K\approx2.3$\,K\,m$^{-1}$;
with bedrock (teal, $K$ some $200\times$ larger) it flattens in the rock and
continues nearly level toward the deep interior. The two part only below the
regolith --- beneath every sensor --- so a base layer leaves the retrieved $K_d$
unchanged.}
\label{fig:baselayer}
\end{figure}
