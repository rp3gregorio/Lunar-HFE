...
cycle-mean profile $\Tm(z;K_d)$ in $\sim$6.9\,s.
...
cycles, $\sim$6.9\,s per trial $K_d$.
...
when the anchor temperature stops moving by more than $0.005$\,K (typically
6--10 cycles).}
...
$\sim$1\,s per Step~A, JIT-compiled (the full $\sim$10-cycle solve is $\sim$6.9\,s).
...
outer cycles, each a $\sim$1\,s Step~A (4--12 inner lunations) plus a microsecond Step~B, take the
...
($\sim$6.9\,s), versus $\sim$1000$+$ lunations for brute force.}
\label{fig:newfilm}
\end{figure}
...
state in $\sim$6.9\,s (measured, median of three runs). Brute-force time
integration reaches the \emph{same} state only on the diffusive timescale: at the
production grid it costs $\sim$0.082\,s per simulated lunation (measured), so the
$\sim$3000 lunations needed just to come within $\sim$0.1\,K of the shortcut take
$\sim$245\,s --- and even then the brute-force profile is \emph{less} converged
than the shortcut's $3$\,mK (Fig.~\ref{fig:overlay}). That is a per-solve speed-up of
\[
\frac{T_{\rm brute}}{T_{\rm anchored}}
\;\approx\;\frac{\sim 245\,\text{s}}{\sim 6.9\,\text{s}}
\;\approx\;\boxed{35\times.}
\]
The retrieval calls the solver $28\times2=56$ times (the $K_d$ sweep at both
sites), so this turns a $\sim$3.8-hour brute-force sweep into a $\sim$6.5-minute
...
$\sim$35$\times$ per-solve saving the per-site retrieval --- and the bootstrap
...
\paragraph{\textbf{Two different speed-ups, not one.}} The $\sim$35$\times$ above
comes from doing \emph{fewer steps}: the flux-anchored loop reaches steady state in
$\sim$100 lunations of forward solving instead of $\sim$3000. A separate, much
...
the JIT disabled). It is only $\sim$1.5$\times$, not the order of magnitude one
might guess, because everything else in the hourly step --- the matrix assembly,
the Newton surface solve, the cycle-mean accumulation --- is already vectorised
NumPy running at C speed. The two gains are orthogonal: the method removes
\emph{steps}; the JIT shaves the \emph{cost per step}.
...
solve: $\sim$6.9\,s vs the $\sim$245\,s a 3000-lunation brute-force run needs to
approach the same state. The full $K_d$ sweep (56 solves) is $\sim$6.5 minutes vs
$\sim$3.8 hours --- about $35\times$. The bootstrap is omitted because it re-uses
...
converged in $\sim$6.9\,s. An animated version (\code{docs/guidebook/old\_vs\_new.gif})
...
the slope method reaches the identical answer in $\sim$6.9\,s from either guess.
...
$\sim$6.9\,s from either guess. Apollo~15, $K_d=4.58$.}
\label{fig:overlay}
\end{figure}
...
$\sim$3000 lunations ($\sim$245\,s) to approach the steady state the
flux-anchored loop certifies in $\sim$6.9\,s --- about $35\times$ faster --- and
across the 56-solve sweep that is $\sim$6.5 minutes versus $\sim$3.8 hours (the
...
The 95\% intervals, $[4.12,\,7.45]$ at A15 and $[6.85,\,9.04]$ at A17,
