...
def extract_sensor_stability(mission: str, min_depth_cm: float) -> dict[str, Any]:
    ...
    for probe_num, dtab in [(1, d1), (2, d2)]:
        ...
        for sensor in np.unique(dtab['sensor']):
            ...
            # First two characters of the sensor name are its bridge type
            # (e.g. 'TG' = gradient bridge, 'TR' = ring bridge) -- the
            # split the letter's TG-vs-TR cross-prediction check groups on.
    ...
    # Excludes shallow sensors biased by the borestem's own thermal
    # signature (self-heating / conduction along the fiberglass-epoxy
    # tube); only sensors below min_depth_cm enter the K_d fit.
