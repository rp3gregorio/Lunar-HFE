...
01 & \nb{01\_methods}    & letter Figs 1--4 & \code{pipeline/figures/} (4 generators) \\
...
04 & \nb{04\_results}    & letter Figs 5--9 & \code{make\_results\_figures}, \code{compute\_diviner\_closure} \\
05 & \nb{05\_discussion} & letter Figs 10--11 & \code{make\_prior\_estimates\_figure} \\
...
\item \emph{(one-time)} \fn{pipeline/fetch\_diviner.py} downloads the Diviner tiles used by Fig 9.
\end{itemize}

\subsection{\nb{01\_methods} --- the methods figures (Figs 1--4)}
...
\item Fig 4 amplitude vs depth \arr\ \fn{make\_letter\_figures.fig\_amplitude\_vs\_depth()} --- diurnal-wave decay (\code{lunar.solver}).
\end{itemize}
...
\subsection{\nb{04\_results} --- the results figures (Figs 5--9)}
Same generator \arr\ \code{show\_fig()} pattern; these read the JSON from \nb{03}:
\begin{itemize}
\item Fig 5 mean-T profile \arr\ \fn{make\_letter\_figures.fig\_mean\_T\_profile()}
\item Fig 6 K\_d sweep \arr\ \fn{make\_letter\_figures.fig\_kd\_sweep()}
\item Fig 7 bootstrap \arr\ \fn{make\_results\_figures.fig\_bootstrap()}
\item Fig 8 thermal profiles \arr\ \fn{make\_results\_figures.fig\_thermal\_profiles()}
\item Fig 9 Diviner closure \arr\ runs \fn{pipeline/compute/compute\_diviner\_closure.py} (model vs Diviner; needs the fetched data).
\end{itemize}

\subsection{\nb{05\_discussion} --- the discussion figures (Figs 10--11)}
\begin{itemize}
\item Fig 10 robustness \arr\ \fn{make\_results\_figures.fig\_robustness()} --- the inter-site contrast under stress tests.
\item Fig 11 five-decade K\_d context \arr\ \fn{make\_prior\_estimates\_figure.main()} --- the two-panel figure separating contact-asymptote K\_d from effective K.
\end{itemize}
