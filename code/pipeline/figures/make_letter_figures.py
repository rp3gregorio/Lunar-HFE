...
Outputs (overwriting the old notebook versions):
  results/figures/fig_apollo_mean_T_profile.pdf
  results/figures/fig_amplitude_vs_depth.pdf
  results/figures/fig_kd_sweep.pdf
"""
...
def fig_amplitude_vs_depth():
    ...
    fig, axes = plt.subplots(1, 2, figsize=(JGR_FULL, 4.4), sharey=True,
                             gridspec_kw={"wspace": 0.06})
    fig.subplots_adjust(left=0.075, right=0.975, top=0.92, bottom=0.10)
    XLIM = (2e-2, 150.0)
    YLIM = (210.0, 0.0)
    ...
    for ax, name in zip(axes, ["A15", "A17"]):
        ...
        # open class carries the heat-short excess this figure diagnoses.
        stem = z_obs < 80.0
        # dotted leaders: model-predicted amplitude -> observed, at fixed
        # depth. The horizontal run of each leader IS the heat-short excess.
        for z_i, a_i in zip(z_obs[stem], amp_obs[stem]):
        ...
        if name == "A15":
            ...
            ax.text(0.1, 170, "digitization noise floor", rotation=90,
                    ha="center", va="center", fontsize=FS_TICK - 1.5,
                    color=C_DIM, style="italic")
            ax.text(XLIM[0] * 1.3, 11, "borestem zone (z < 80 cm)",
                    fontsize=FS_TICK, color=C_DIM, va="center", style="italic")
        else:
            # name the diagnostic once, on the longest leader (deepest
            # borestem sensor), in the verified-empty mid-panel pocket
            i_deep = int(np.argmax(z_obs[stem]))
            z_x = z_obs[stem][i_deep]
            a_x = amp_obs[stem][i_deep]
            a_m = max(100.0 * np.exp(-z_x / delta_H), XLIM[0])
            ax.annotate("heat-short excess",
                        xy=(np.sqrt(a_m * a_x), z_x), xytext=(0.32, 112),
                        fontsize=FS_TICK - 1, color=C_DIM, style="italic",
                        ha="left", va="center",
...
def fig_kd_sweep():
    ...
    # PCHIP (shape-preserving) not CubicSpline: the dense non-uniform K_d grid
    # (0.2 mW near the minimum, coarse elsewhere) makes a natural cubic spline
    # overshoot at the spacing transition, producing spurious spikes near 3.5 mW.
    ...
    # Feng et al. (2020) deep value, verified from the paper: kd = 3.8e-3 W/m/K
    ...
    for name, color in [("A15", C_A15), ("A17", C_A17)]:
        ...
        cs       = PchipInterpolator(kd_all, rm_all)
