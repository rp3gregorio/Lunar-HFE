...
| `01_methods.ipynb` → `04_discussion.ipynb` | the four reproduction notebooks, in paper order (methods → retrieval → results → discussion) |
| `05_animations.ipynb` | GIF builders |
| `equilibrium_demo.ipynb` | the teaching demo: brute force converges onto the flux-anchored answer; instantaneous profiles via `profile_at_time` / `profile_at_local_time` |
...
| run one converged solve in Python | see the last cells of `notebooks/equilibrium_demo.ipynb` |
