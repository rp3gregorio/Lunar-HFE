...
- PASS: Notebook spot-check: `notebooks/00..04` contain no inline solver, stable-window, or `K(T,z)` reimplementation; stale `scripts/`, `output/`, and `paper/letter/figures/` paths were updated.
- PASS: Letter audit completed without editing `paper/letter/letter.tex` or `letter.pdf`.
...
- Letter: wrote `paper/letter/AUDIT_20260622.md` only.

## Not Fixed

- HIGH: `paper/letter/letter.tex:826` labels Apollo 15 Table 2 as `K_d*=4.57 [4.12,7.45]`; canonical retrieval text/JSON round to 4.58. Recommendation: change to 4.58 or relabel as the RMSE-table site-fit value.
- MEDIUM: `paper/letter/letter.tex:722` still says each `K_d` uses a full Crank--Nicolson spin-up, while the solver section describes the flux-anchored equilibrium construction. Recommendation: revise to “certified periodic-equilibrium solve.”
...
See `paper/letter/AUDIT_20260622.md`.
