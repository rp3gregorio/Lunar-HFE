...
def fig_thermal_profiles(d, out_path):
    ...
    (lunar.properties).  Runs the forward model from scratch."""
    import sys; sys.path.insert(0, str(pathlib.Path(__file__).parents[2]))
    from lunar.apollo_helpers import extract_sensor_stability

    # Site metadata only -- the physics (grid, forcing, conductivity, Q_b)
    # lives in lunar.config / pipeline.compute.retrieve_kd, which this
    # figure calls below.  The old local forward-model helpers here (a
    # fixed 30-lunation spin-up and an unverified Martinez B1 variant)
    # were superseded and deleted; git history has them if ever needed.
    SITE_CFGS = {
    ...
    # coefficients in lunar.properties).
