...
\begin{tikzpicture}[
  ...
  \node[tbox, below=4.5mm of m1] (m2)
    {\textbf{score it --- prior}:\quad $Q_b \sim \mathcal{N}(\text{published},\,15\%)$;
     \; $K_d$ flat in $\log$ over $[1, 30]$ mW\,m$^{-1}$\,K$^{-1}$};
  \node[tbox, below=4.5mm of m2] (m3)
    {\textbf{score it --- likelihood}: read RMSE$(K_d, Q_b)$ off a surface
     of $\sim$200 \emph{real} solves (13 $K_d$ $\times$ 9 $Q_b$ grid;
  ...
  \node[res, below=5mm of post]
    {the cloud is a \emph{ridge} whose tilt is measured, not assumed:\\
     at A15 higher $Q_b$ trades against \emph{higher} $K_d$, at A17 against
     \emph{lower} --- and $P(K_d^{\rm A17}>K_d^{\rm A15})\approx99\%$
     however wide the $Q_b$ prior};
