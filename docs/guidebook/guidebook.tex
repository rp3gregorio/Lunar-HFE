...
\caption{Brute force converges onto the flux-anchored shortcut. \textbf{(a)} Two
brute-force runs from very different starting guesses (240\,K, 260\,K) both
approach the \emph{same} shortcut equilibrium (dashed) as the spin-up lengthens.
\textbf{(b)} Their maximum difference from the shortcut below $0.3$\,m decays
toward the shortcut's own $0.03$\,K tolerance --- but only after several hundred
lunations, whereas the slope method lands there in $\sim$9\,s from either guess.
Apollo~15, $K_d=4.58$.}
\label{fig:overlay}
\end{figure}
