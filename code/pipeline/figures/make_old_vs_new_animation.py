...
Two persistent panels (all curves real solver output):
  ...
  RIGHT -- Step A settles the skin, the anchor drops at z0 = 0.55 m, then the
           ...
           column; converged in ~10 cycles, ~75 s (0.6 s compiled).
...
shows the measured ~21x (one solve: ~26 min brute vs ~75 s flux-anchored, interpreted tier).
...
SITE = SITES["A15"]; KD = 4.60e-3; GUESS = 240.0
...
def main():
    ...
    specs += [dict(act=2, old=OLD_FINAL, new=("converged", None), pulse=6, old_t=245, new_t=14.0,
                   cap="A few refine cycles polish it -- converged in ~10 cycles, ~75 s")] * 4
    specs += [dict(act=3, old=OLD_FINAL, new=("converged", None), badge=True, pulse=6, old_t=245, new_t=14.0,
                   cap="Same steady state, ~21x faster: ~26 min brute force  vs  ~75 s flux-anchored")] * 18
    ...
    def draw_new(kind, k, pulse, badge):
        ...
        if kind == "start":
            ...
            if kind == "converged":
                ...
                axN.text(0.04, 0.12, "converged\n~10 cycles · ~75 s", transform=axN.transAxes,
    ...
    cols = [("old", bf30, None, "old: 30 lunations"),
            ...
            ("new", "converged", None, "new: converged (~75 s)")]
    ...
    fig.suptitle("Reaching steady state: brute force (~1000 lunations, still ~1 K off) vs the "
                 "flux-anchored anchor + slope walk (~75 s)  ·  bottom row = deep zoom",
