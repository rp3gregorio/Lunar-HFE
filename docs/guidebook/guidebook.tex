...
converged in $\sim$9\,s. An animated version (\code{docs/guidebook/old\_vs\_new.gif})
...
\caption{With vs without a bedrock base layer, integrating the steady-state
closure $d\langle T\rangle/dz=Q_b/K$ down from the Apollo~15 anchor.
\emph{Without bedrock} (green, this study) the profile rises almost linearly at
the geothermal gradient $Q_b/K\approx2.3$\,K\,m$^{-1}$ and never settles.
\emph{With a bedrock base} below $5$\,m (teal) the conductivity jumps
$\sim$200$\times$, the gradient nearly vanishes, and the profile flattens toward
a constant; $Q_b=0$ (dotted) would make it isothermal. The seven Apollo~15 deep
sensors (with $1\sigma$ bars) lie at $0.8$--$1.4$\,m, entirely within the
regolith, where the two models are identical and fit the data equally
(inset) --- so a base layer changes only the profile \emph{below the data} and
leaves $K_d$ unchanged.}
\label{fig:baselayer}
\end{figure}
