...
\begin{tikzpicture}[
  ...
  cnd/.style={rectangle, draw=coral, line width=0.9pt, fill=coral!7,
              inner sep=4pt, text width=54mm, anchor=north west},
...
\node[prb, below=3mm of pA.south west, anchor=north west,
  ...
  to a certified periodic steady state\\[2pt]
  {\color{forest}\bfseries this is where the anchor method acts}};
% the anchor method is only legal under one condition; state it where the
% method is invoked rather than leaving it to the chapter
\node[cnd, below=2mm of pB.south west, anchor=north west] (pCond) {%
  {\color{coral}\textbf{Condition}} --- legal only once the cycle
  \emph{closes}: over one lunation $\langle\partial_t T\rangle=0$, so
  $\langle K\,\partial_z T\rangle=Q_b$ at \emph{every} depth.\\[1pt]
  {\color{dim}Anchor $z_0=0.55$ m, below the rectification zone
   ($u_{\rm rect}<1\%$ of $Q_b$).}};
\node[prb, below=3mm of pCond.south west, anchor=north west] (pC) {%
...
\node[hl]  at (12.4,-0.4)  (r1) {%
  retrieved $K_d^{*}$\\[1pt]
  \textbf{\itshape A15:\; 4.60}\\
  \textbf{\itshape A17:\; 7.08}};
\node[rsb, below=2mm of r1.south west, anchor=north west] (r2) {%
  95\% bootstrap CI\\
  \itshape A15:\,[4.18,\,6.96]\\
  \itshape A17:\,[6.16,\,8.07]};
\node[rsb, below=2mm of r2.south west, anchor=north west] (r3) {%
  inter-site contrast\\
  \itshape A17 $-$ A15 $= 2.31$\\
  {\color{dim}bootstrap tail $0.031$}};
...
\draw[arr] (pB.south) -- (pCond.north);
\draw[arr] (pCond.south) -- (pC.north);
\draw[arr] (pC.south) -- (pD.north);

% --- arrows: process to results ---
% Fix (2026-07): a direct diagonal (pC.east)--(r1.west) cut straight across
% the top of box D and crossed the three (pD.east)--(rN.west) arrows below
% it, producing a tangled X in the gap between the PROCESS and RESULTS
% columns. Routed instead as two parallel orthogonal "bus" trunks — one at
% +3mm off the process column for C's single result, one at +7mm for D's
% three — so each source box gets its own vertical lane and no two arrows
% cross.
\draw[arr] (pC.east) -- ++(3mm,0) |- (r1.west);
\draw[arr] (pD.east) -- ++(7mm,0) |- (r2.west);
\draw[arr] (pD.east) -- ++(7mm,0) |- (r3.west);
\draw[arr] (pD.east) -- ++(7mm,0) |- (r4.west);
\end{tikzpicture}
