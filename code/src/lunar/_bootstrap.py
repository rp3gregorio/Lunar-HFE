...
def find_repo_root(start: pathlib.Path | None = None) -> pathlib.Path:
    ...
    Returns the directory that holds the machinery — ``src/``, ``data/``,
    ``results/`` — since callers build paths like ``root/"data"`` and
    ``root/"results"`` off it. Three layouts are recognised, newest first:

      * umbrella layout  <root>/code/{src/lunar, data, results}   → ``code/``
      * classic src-layout  <root>/{pyproject.toml, src/lunar}    → ``<root>``
      * flat  <root>/{pyproject.toml, lunar}                      → ``<root>``
    """
    here = (start or pathlib.Path.cwd()).resolve()
    for candidate in (here, *here.parents):
        # umbrella layout (2026-07-06 reorg): the package + data + results
        # live under a `code/` umbrella, with pyproject.toml at the repo root.
        if (candidate / "code" / "src" / "lunar" / "__init__.py").is_file():
            return candidate / "code"
        if (candidate / "src" / "lunar" / "__init__.py").is_file() \
                and (candidate / "results").exists():
            return candidate            # already inside code/ (or classic root)
