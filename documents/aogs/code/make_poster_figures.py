...
OUT = AOGS / "figures" / "poster"; OUT.mkdir(parents=True, exist_ok=True)
...
def emit_numbers(sens, study):
    ...
    xp = AOGS / "results" / "aogs_crossite.json"
    if xp.exists():
    ...
    sp = AOGS / "results" / "aogs_svf_terms.json"
    if sp.exists():
    ...
    bp = AOGS / "results" / "aogs_winner_bootstrap.json"
    if bp.exists():
...
def transfer_matrix():
    ...
    x = json.loads((AOGS / "results" / "aogs_crossite.json").read_text())
...
def bootstrap_panel():
    ...
    boot = json.loads((AOGS / "results" / "aogs_winner_bootstrap.json").read_text())
    xs = json.loads((AOGS / "results" / "aogs_crossite.json").read_text())
...
def main():
    sens = json.loads((AOGS / "results" / "aogs_sensitivity.json")
                      .read_text())
    dem_panels(sens)
    horizon_panels(sens)
    forcing_panels(sens)
    sites_composite(sens)
    study_p = AOGS / "results" / "aogs_density_study.json"
    if study_p.exists():
        ...
        density_mini(study)
        if (AOGS / "results" / "aogs_crossite.json").exists():
            transfer_matrix()
        if (AOGS / "results" / "aogs_winner_bootstrap.json").exists():
            bootstrap_panel()
