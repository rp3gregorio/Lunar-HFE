...
(1973) supplies $c_p(T)$. Fix the five Hayne constants and the density profile,
...
With $K(T,z)$ chosen, we solve the heat equation in the column,
\begin{equation}
...
\label{eq:bottombc}
\end{equation}
No closed-form solution exists when $K$ depends on both $T$ and $z$, so we
discretise: a
...
centimetres by $1$\,m). The dimensionless cell-to-cell coupling per step,
...
\label{eq:vertex}
\end{equation}
giving sub-grid precision with no extra (9-second) solves. For Apollo~15 the
...
the bootstrap pretends we can. Treat the seven sensors as the population, draw
...
obeys a simple first-order ODE in depth:
\begin{equation}
...
already settled. The whole method is two operations, alternated
...
midpoint rule (Runge--Kutta order 2), one cell at a time. To see it concretely,
...
toward its fixed value. A representative trace of the anchor temperature and its
change between cycles (``drift''):
...
\midrule
1 ($z_0{=}0.25$\,m) & 1 & 251.2  & --- \\
                    & 2 & 253.0  & 1.8 \\
                    & 3 & 253.2  & 0.20 \\
                    & 4 & 253.24 & 0.04$^{\dagger}$ \\
\midrule
2 ($z_0{=}0.55$\,m) & 1 & 253.70  & --- \\
                    & 2 & 253.90  & 0.20 \\
                    & 3 & 253.96  & 0.06 \\
                    & 4 & 253.978 & 0.018 \\
                    & 5 & 253.982 & 0.004$^{\ddagger}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\smallskip
\noindent The drift shrinks by roughly a factor of $\sim$3--5 each cycle --- the
geometric contraction the fixed-point argument guarantees. Nine outer cycles,
each a $\sim$9--12\,s Step~A plus a microsecond Step~B, take the anchor to a
$4$\,mK drift; the certified solve then passes all three checks of
\S\ref{sec:cert}. Run the \emph{same} loop from a 240\,K guess instead of
260\,K and it lands within $0.038$\,K of this profile at every depth --- that is
the guess-independence number. Figure~\ref{fig:newfilm} shows the same solve as
...
\section{The 700$\times$ speed-up, in one calculation}\label{sec:speedup}

The payoff is the wall-clock saving. Compare the two routes through the same
retrieval loops:
\[
\text{Brute force}:\;
\underbrace{1000\,\text{lunations}}_{\tau\sim z_{\max}^2/\kappa}
\times
\underbrace{9\,\text{s/lun}}_{\text{JIT solver}}
\times
\underbrace{28\,K_d}_{\text{sweep}}
\times
\underbrace{2\,\text{sites}}_{}
\times
\underbrace{1500\,\text{boots}}_{}
\;\sim\;
2\,\text{years.}
\]
\[
\text{Flux-anchored}:\;
\underbrace{6\text{--}10\,\text{cycles}}_{B\circ A}
\times
\underbrace{12\,\text{s}}_{\text{Step A}}
\times
\underbrace{28\times2\times1500}_{\text{same loops}}
\;\sim\;
\underbrace{12\,\text{hours.}}_{\text{tractable}}
\]
The ratio is about $700\times$. Without it, the per-site retrieval --- and the
bootstrap that gives it a confidence interval --- would simply not have been
possible on commodity hardware. Ch.~\ref{ch:bruteforce} unpacks what this
comparison means.
...
When brute force is given \emph{enough} time --- not 30 lunations but the full
$\sim$1000 --- it converges to the same profile the slope method produces in
seconds. We verified this directly. Run the slope method from three wildly
different starting temperatures (200, 250, 300\,K): all three land on the same
profile to within 21\,mK at sensor depths (Fig.~\ref{fig:overlay}b,c). Run the
old method from two guesses (200, 250\,K) at 30 lunations: they disagree by
$\sim$40\,K (Fig.~\ref{fig:overlay}a). The error of a brute-force run falls as a
power law in lunation count (Fig.~\ref{fig:conv}); at 30 lunations it is still
$\sim$13\,K, and only after hundreds of lunations does it reach the sub-30\,mK
band the slope method hits in $\sim$9\,s. The two methods agree --- the slope
method just refuses to wait.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth]{figures/fig_equilibrium_demo.pdf}
\caption{Guess-dependence test. \textbf{(a)} The old method (30 lunations) from
two starting guesses gives different answers --- a $\sim$40\,K gap at sensor
depths. \textbf{(b)} The slope method from three guesses (200, 250, 300\,K) lands
on the same profile. \textbf{(c)} Residual: the slope method's three runs agree
to $<$21\,mK at sensor depths; the old method's error exceeds 12\,K. This is the
proof that the slope method converges to a unique, guess-independent steady
state.}
\label{fig:overlay}
\end{figure}
...
\caption{Brute-force error at sensor depths (0.8--2.4\,m) vs.\ lunation count. At
30 lunations (red dashed) the old method still carries $>$12\,K of error. The
shaded band marks the $<$30\,mK tolerance the slope method reaches in $\sim$9\,s.}
\label{fig:conv}
\end{figure}

\section{The wall-clock difference}

We did the arithmetic in \S\ref{sec:speedup}: brute force across the full
retrieval ($1000$ lunations $\times$ the sweep $\times$ two sites $\times$ 1500
bootstraps) runs to about two years of compute; the flux-anchored loop, doing the
\emph{same} outer loops with $\sim$10 cheap cycles in place of 1000 lunations,
finishes in about twelve hours --- roughly $700\times$ faster. The point for this
comparison is \emph{where} the saving comes from: it is not a faster inner step
...
recorded; the independent contributions add in quadrature:
