...
bars), the two models sit exactly on top of each other and fit the data equally.
\emph{Right:} over the whole column they part only below $5$\,m --- without
bedrock (green, this study) the profile keeps rising at the geothermal gradient
$Q_b/K\approx2.3$\,K\,m$^{-1}$, while a bedrock base (teal, $K$ some
$200\times$ larger) flattens it toward a constant. Because the split sits below
every sensor, a base layer leaves the retrieved $K_d$ unchanged.}
\label{fig:baselayer}
\end{figure}
